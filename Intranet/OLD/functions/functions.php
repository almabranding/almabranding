...
class View{
    ...
    public function View($o){
        $this->options=$o;
    }
    public function viewGrill($t,$l) 
    { 
        $this->title=$t;
        $this->list=$l;
        ...
        foreach ($this->title as $id=>$value){
            ...
            $view.="<li style='width:".$width.";'>".$value['value']."</li>";
        }
        $view.='</ul></li><li><ul class="'.$sortable.'">';
        foreach ($this->list as $id=>$value){
            if($value['_link']) $view.='<a href="'.$value['_link'].'">';
            $view.='<li><ul class="colsView">';
            $i=0;
